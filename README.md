...
* 커스텀훅은 리액트쿼리에서 모범사례
* @testing-library/react-hooks: react훅을 테스트하기위한 전용라이브러리
* 훅테스트와 컴포넌트 데스트 다르게 있음
* 각 테스트 마다 자체 queryClient가 있어야 테스트를 깔끔하고 확실하게 만들 수 있다 -> 테스트와 쿼리 클라이언트간에 크로스 오버가 발생하지 않도록 하기 위함
* 내부에서 사용자 상호작용을 테스트하는 것은 일반적으로 모범 사례로 여겨진다.
* 그냥 내가  보면서 체험해보는게 좋을듯 ?
* 오류가 화면에 출력되지 않도록 해야함 -> 그렇지 않으면 작동중인 테스트에 경보가 발생할 수 있기 때문
* 재시도를 억제해야함 -> 테스트가 타임아웃 되지 않도록 쿼리 클라이언트의 기본값을 업데이트하여 제시도를 억제해야함 -> 앱이 재시도에 어떻게 응답하는지 테스트하는 경우 그에 맞게 재시도를 설정하는것이 좋음
* 오류테스트의 경우 reactquery의 setLogger 함수를 사용하는것이 좋다
*  강의 완료!!